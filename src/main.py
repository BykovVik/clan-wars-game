...
async def text_handler(client, message):
    ...
    if message.text == "/start" and message.chat.type.name == "PRIVATE":
        await message.reply_text('Это первый бот для социальной игры в Телеграм!\n\nСпециально для тебя, мы дали возможность бросить вызов любому чату⚔️ в Телеграме 👊\n\nИнтересно? Чтобы начать, ДОБАВЬ меня в любой ЧАТ и попроси дать мне права АДМИНИСТРАТОРА.\n\n Создай свою тусовку в телеграме и стань первым!👊🏻 ')
    ...
    if message.text == "/help" and not message.chat.type.name == "PRIVATE":
        ...
        if clan is None:
            return await message.reply_text("В вашем чате НЕ создан клан, нажмите команду `/start` и следуйте дальнейшим инструкциям")
        
        if user is None:
            #create button
            button = InlineKeyboardButton("Играть за этот ЧАТ", callback_data="add_player")
            keyboard = InlineKeyboardMarkup([[button]])
            #message reply
            await message.reply_text(
            ...
            #create button
            button = InlineKeyboardButton("Удалить аккаунт", callback_data="delete_player")
            keyboard = InlineKeyboardMarkup([[button]])
            #message reply
            await message.reply_text(
    ...
    if message.text == "/info" and message.chat.type.name == "PRIVATE":
        #create button
        button_rules = InlineKeyboardButton("Прочитать правила игры", callback_data="rules_of_the_game")
        button_account = InlineKeyboardButton("Аккаунт", callback_data="account")
    ...
    if message.text[:6] == "/check" and not message.chat.type.name == "PRIVATE":
        if len(message.text) == 6:
            #message reply
            return await message.reply_text("Введите команду `/check`, а за ней впишите название чата, с которым вы хотите сразиться. Если такой чат зарегистрировал свой КЛАН в игре, то произойдет приглашение выбранного КЛАНА для игры с КЛАНом, в котором состоите вы")
        
        if clan is None:
            #message reply
            return await message.reply_text("КЛАН с таким именем найден зарегестрирован в игре, однако в вашем чате НЕ создан клан, нажмите команду `/start` и следуйте дальнейшим инструкциям")
        
        title = message.text[7:]

        if title == clan.title:
            #message reply
            return await message.reply_text("Нельзя вызвать на бой, свой же КЛАН.")
        ...
        if check_war is None:
            #message reply
            await message.reply_text("Клан с подобным именем не найден, возможно вы допустили ошибку.")
        else:
            #create button
            button = InlineKeyboardButton("Вызвать КЛАН", callback_data="battle_{}".format(check_war.chat_id))
            keyboard = InlineKeyboardMarkup([[button]])
            #message reply`
            await message.reply_text(
                "КЛАН с таким именем найден зарегестрирован в игре, хотите бросить ему вызов? 👊🏻 ", reply_markup=keyboard       
            )
...
async def callback_answers(client, callback_query):
    ...
    if callback_query.data == 'add_player':
        if user is None:
            ...
            await callback_query.answer("Теперь вы играете за чат в котором вы сейчас находитесь")
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
        else:
            await callback_query.answer("Вы уже являетесь участником КЛАНа")
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
    ...
    if callback_query.data == 'delete_player' and user is not None:
        if user is not None:
            ...
            await callback_query.answer("Ваш аккаунт не зарегистрирован в игре")
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
    ...
    if callback_query.data == 'rules_of_the_game':
        ...
        await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
        await client.send_message(callback_query.message.chat.id, "Правила игры")
    ...
    if callback_query.data == 'account':
        if user is None:
            ...
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
            await client.send_message(callback_query.message.chat.id, "Для того что бы заркгистрироваться в игре, вам необходимо выбрать чат за который вы хотите играть, вызвать в нём команду `/help`, и зарегистрировать своего юзера в игре.")
        else:
            await callback_query.answer("Информация о игроке")
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
            await client.send_message(callback_query.message.chat.id, "Юзер инфо")
    ...
    if callback_query.data[:6] == "battle":
        if user is not None:
            if user.clan_id == clan.id:
                ...
                await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
                #create new game
                game = GameController(attacking_clan_id, defending_clan_id, client)
                await game.start_game()
            else:
                await callback_query.answer("Вы не можите проявлять активность в чате, за который вы не играете!")
        else:
            await callback_query.answer("Вы не можите проявлять активность. Ваш аккаунт не зарегистрирован в игре")
            await client.delete_messages(callback_query.message.chat.id, callback_query.message.id)
